...
// KEY_WOW64_64KEY explicitly. URmessage.exe is x64 so it would get the 64-bit view
...
std::string DescribeCrashDumpChannel(const CrashDumpChannel& channel,
  ...
  if (channel.werDisabled) {
    return std::format(
        "app: NATIVE-FAULT EVIDENCE IS OFF AT THE MACHINE LEVEL. "
  ...
  const std::string fix = std::format(
      ...
      "%LOCALAPPDATA%\\URmessage\\app\\crashdumps /f  &&  reg add "
      "\"HKLM\\SOFTWARE\\Microsoft\\Windows\\Windows Error "
      "Reporting\\LocalDumps\\{}\" /v DumpType /t REG_DWORD /d 2 /f  &&  reg add "
      "\"HKLM\\SOFTWARE\\Microsoft\\Windows\\Windows Error "
      "Reporting\\LocalDumps\\{}\" /v DumpCount /t REG_DWORD /d 5 /f   "
      "(DumpType 2 is a FULL dump. A mini dump of a WinUI/XAML fault routinely "
      "lacks the frames that name it: the interesting stack is usually on a "
      "runtime thread this process never created. Takes effect on the next "
      "fault — no restart, no reinstall.)",
  ...
  if (!channel.localDumpsForThisExe && !channel.localDumpsGlobalHasValues) {
    ...
    return std::format(
        "app: crash-dump channel — WER will report a native fault but is "
  ...
  // Configured. Say exactly where the file will land. URmessage runs as the
  // interactive user (app.manifest: asInvoker), so the Windows default really
  // is the operator's own %LOCALAPPDATA%\CrashDumps — unlike the VPN client's
  // LocalSystem service, whose %LOCALAPPDATA% resolves under
  // C:\Windows\System32\config\systemprofile and is why that codebase spells
  // the default out. Kept explicit here anyway: "the default" is not a path.
  const std::string folder =
      channel.dumpFolder.empty()
          ? std::string(
                "%LOCALAPPDATA%\\CrashDumps (the Windows default; this process "
                "runs as the interactive user, so that is the signed-in "
                "profile's AppData\\Local)")
          : Narrow(channel.dumpFolder);

  return std::format(
      "app: crash-dump channel OPEN — a native fault should write a dump to "
