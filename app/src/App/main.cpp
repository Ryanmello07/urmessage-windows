// URmessage entry point. Single-instance, registers the AppUserModelId (for
// toasts and taskbar grouping), then hands off to the WinUI 3 Application.
//
// Adapted from the URnetwork Windows VPN client's App/main.cpp. What is kept is
// the whole failure-visibility apparatus, because every one of those cases is a
// property of an unpackaged WinUI 3 process and not of a VPN: the log opened as
// the first instruction, the diagnose switch, the redirect timeout, and a
// message box on every path that can die before there is any UI.
//
// What is gone: the wallet/OAuth deep-link commentary and the `--relaunched`
// update handoff, which belong to the updater this app does not have yet.
//
// SINGLE-INSTANCING IS THE SHARP PART. It uses the Windows App SDK's
// AppInstance, keyed by STRING, and the key lives in Common/Ids.h. If that key
// were the VPN client's "URnetwork.Desktop", then launching URmessage while the
// VPN client is running would not start URmessage — it would redirect this
// activation into the VPN client's window and exit 0, and the only evidence
// would be a line in the other app's log.
//
...
// if the wait itself fails, which also removes the message pumping this
...
// symptom this apparatus exists to remove, and "nothing happened, so I ran it
// again" is exactly how a user gets there.
...
bool RedirectActivation(AppInstance const& primary, AppActivationArguments const& args) {
  ...
  if (!state->done) {
    urnw::FailVisible(
        L"URmessage is already running, but this launch could not hand over to it.",
  ...
  if (hr == RPC_S_CALLPENDING) {
    urnw::FailVisible(
        L"URmessage is already running, but it did not respond.\n\n"
        L"If it is not responding, end URmessage.exe from Task Manager and "
        L"start it again.",
        std::format(L"The activation redirect timed out after {} ms.",
                    kRedirectTimeoutMs));
  } else if (FAILED(hr)) {
    urnw::FailVisible(
        L"URmessage is already running, but this launch could not reach it.",
        std::format(L"CoWaitForMultipleObjects failed: HRESULT 0x{:08X}",
                    static_cast<uint32_t>(hr)));
  } else {
    urnw::FailVisible(
        L"URmessage is already running, but it refused this launch's request.",
...
int __stdcall wWinMain(HINSTANCE, HINSTANCE, PWSTR, int) {
  ...
  try {
    ...
    ::MessageBoxW(nullptr,
                  L"URmessage could not start: its own startup diagnostics failed.\n\n"
                  L"This usually means %LOCALAPPDATA% is not writable for this user.",
                  L"URmessage could not start",
  ...
  // Not fatal on its own (it costs toast delivery and taskbar grouping), so it
  ...
  // Application::Start does not return until the app exits.
  if (!urnw::WasLaunched()) {
    urnw::FailVisible(
        L"URmessage started but its user interface never launched, so it has "
        L"exited.\n\n"
        L"The app's resources (resources.pri, next to URmessage.exe) are the "
        L"usual cause.",
