...
// HKCU\Software\URmessage\Window, value "Placement": ONE REG_BINARY blob.
//
// The subkey comes from Ids.h rather than being spelled here, because it is one
// of the identities that MUST differ from the URnetwork VPN client's
// (Software\URnetwork\Window). Left shared, the two apps would restore each
// other's geometry — and the blob carries a magic + version, so the reader
// would ACCEPT it: same magic, same version, wrong window.
//
...
constexpr const wchar_t* kPlacementKey = urnw::ids::kWindowPlacementKey;
